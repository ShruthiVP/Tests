...
from tests.lorawan_commands.test_executor import Device
#add Gevent command
from tests.lorawan_commands.commands import Cmd, SendBytes, Lorawan, Stack, Ping, Sevent, Sconfirm, Confirm, Devid, \
...
class TestLorawanCommands:
    ...
    def test_otaa_repeatlwjoin(self, device1,parser_mode):
        device1.send_receive(Sappeui("12 34 56 78 12 34 56 78"))
    ...
    def test_abp_lwjoin(self, device1,parser_mode):
        device1.send_receive(Sappskey("F5 DA C7 34 93 55 3B C2 E6 2D 29 8D 07 CA 8A D3"))

        device1.send_receive(Gappskey("F5 DA C7 34 93 55 3B C2 E6 2D 29 8D 07 CA 8A D3"))

        device1.send_receive(Snwkskey("78 CD 76 3D 8F 2B 24 A0 BD D0 AF D7 EC 7F 76 13"))
        device1.send_receive(Gnwkskey("78 CD 76 3D 8F 2B 24 A0 BD D0 AF D7 EC 7F 76 13"))

        device1.send_receive(Sotaa(enabled=False))
        device1.send_receive(Gotaa(enabled=False))

        if parser_mode == "ascii" or parser_mode == "at":
            device1.send_receive(Sdevaddr("006cf06d"))
            device1.send_receive(Gdevaddr("6CF06D"))
            device1.send_receive(Lwjoin())
            device1.read_event("EVENT Joining the LoRaWAN network...")
            device1.read_event("OK00>")
            device1.read_event("EVENT Device set by ABP")
            pause()
            device1.send_receive(Lwstatus("01"))
            pause()
            device1.send_receive(SendBytes(5, "12345"))
            ...
            device1.send_receive(Sdevaddr("00 6C F0 6D"))
            device1.send_receive(Gdevaddr("00 6C F0 6D"))
            device1.send_receive(Lwjoinbinabp("1b 50 1b 51 01 1b 53 4a 6f 69 6e 69 6e 67 20 74 68 65 20 4c 6f 52 61 57 41 4e 20 6e 65 74 77 6f 72 6b 2e 2e 2e 1b 54 1b 53 44 65 76 69 63 65 20 73 65 74 20 62 79 20 41 42 50 1b 54"))
            pause()
            device1.send_receive(Lwstatus(1))
            pause()
            device1.send_receive(SendBytes(5, "12345"))
